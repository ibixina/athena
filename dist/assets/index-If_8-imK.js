...
- By [elimination](https://en.wikipedia.org/wiki/Proof_by_exhaustion), you can then deduce the identity of the third god.`,bo={id:Ce,title:De,difficulty:Ie,estimatedTime:je,statement:Ee,publishDate:Oe,concepts:We,hints:Me,solution:qe},$o=Object.freeze(Object.defineProperty({__proto__:null,concepts:We,default:bo,difficulty:Ie,estimatedTime:je,hints:Me,id:Ce,publishDate:Oe,solution:qe,statement:Ee,title:De},Symbol.toStringTag,{value:"Module"})),Re="ants-on-a-stick",ze="Ants on a Stick",Ue="Medium",He="30-60 minutes",Ge="100 ants are walking on a meter stick (100 cm). Each ant travels at a constant speed of 1 cm per second. They start at various positions and arbitrary directions. When two ants collide, they instantly reverse direction and continue at the same speed. If an ant reaches either end of the stick, it falls off. What is the maximum possible time before all ants have fallen off the stick, from any starting configuration?",Le="2026-04-12",Ne=[{name:"Invariants and Symmetry",description:"Properties of a system that remain unchanged under some transformations.",resourceTitle:"Invariant",resourceUrl:"https://en.wikipedia.org/wiki/Invariant_(mathematics)",readTime:"10 min"}],Fe=[{order:1,content:"What actually happens to the global system of ants during a collision?"},{order:2,content:"Since the ants are identical, does it matter which ant is which?"},{order:3,content:"Imagine the ants simply pass through each other instead of bouncing. How does this change the macro-state?"}],Ye=`### Step 1: The Insight
...
It is **impossible** to connect them without a crossing. $\\blacksquare$`,ns={id:zi,title:Ui,difficulty:Hi,estimatedTime:Gi,statement:Li,publishDate:Ni,concepts:Fi,hints:Yi,solution:Ji},is=Object.freeze(Object.defineProperty({__proto__:null,concepts:Fi,default:ns,difficulty:Hi,estimatedTime:Gi,hints:Yi,id:zi,publishDate:Ni,solution:Ji,statement:Li,title:Ui},Symbol.toStringTag,{value:"Module"})),os=Object.assign({"../../problems/001-duplicate-subsets.json":lo,"../../problems/002-blue-eyed-islanders.json":co,"../../problems/003-100-prisoners-boxes.json":mo,"../../problems/004-poisoned-wine.json":fo,"../../problems/005-25-horses.json":yo,"../../problems/006-three-gods.json":$o,"../../problems/007-ants-on-a-stick.json":vo,"../../problems/008-12-coin-problem.json":To,"../../problems/009-two-egg-problem.json":So,"../../problems/010-pirate-game.json":Ao,"../../problems/011-mutilated-chessboard.json":Po,"../../problems/012-camel-and-bananas.json":Do,"../../problems/013-100-prisoners-lightbulb.json":jo,"../../problems/014-josephus-problem.json":Oo,"../../problems/015-airplane-seating.json":Mo,"../../problems/016-cheryls-birthday.json":Ro,"../../problems/017-secretary-problem.json":Uo,"../../problems/018-game-of-nim.json":Go,"../../problems/019-prisoners-hat-parity.json":No,"../../problems/020-art-gallery.json":Yo,"../../problems/021-stable-marriage.json":Xo,"../../problems/022-party-handshake.json":Vo,"../../problems/023-st-petersburg.json":Zo,"../../problems/024-bridge-and-torch.json":ts,"../../problems/025-three-utilities.json":is}),_=Object.values(os).map(e=>e.default).sort((e,t)=>e.publishDate.localeCompare(t.publishDate));function ss(){const e=new Date().toISOString().split("T")[0],t=_.find(i=>i.publishDate===e);if(t)return t;const a=(Math.floor((new Date().getTime()-new Date("2026-04-06").getTime())/(1e3*60*60*24))%_.length+_.length)%_.length;return _[a]}const Xi="athena_progress";function C(){try{const e=localStorage.getItem(Xi);if(e)return JSON.parse(e)}catch(e){console.error("Failed to load progress:",e)}return{solvedProblems:[],scratchpad:{},lastVisit:new Date().toISOString().split("T")[0]}}function Ki(e){try{localStorage.setItem(Xi,JSON.stringify(e))}catch(t){console.error("Failed to save progress:",t)}}function as(e){const t=C();t.solvedProblems.includes(e)||(t.solvedProblems.push(e),Ki(t))}function j(e,t){const o=C();o.scratchpad[e]=t,Ki(o)}function E(e){return C().solvedProblems.includes(e)}function rs(e){return C().scratchpad[e]||""}let g,P=new Set,v=!1,p=!1,f=[],n=[33,34,33],d=[];function O(e){if(!e)return"";let t=e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");t=t.replace(/^### (.*$)/gim,"<h4>$1</h4>").replace(/^## (.*$)/gim,"<h3>$1</h3>").replace(/^# (.*$)/gim,"<h2>$1</h2>").replace(/\$(.*?)\$/g,(i,s)=>`<code class="math">${s.replace(/\\dots/g,"&hellip;").replace(/\\blacksquare/g,"&#9632;").replace(/\^\{(.*?)\}/g,"<sup>$1</sup>").replace(/\^(.)/g,"<sup>$1</sup>").replace(/_\{(.*?)\}/g,"<sub>$1</sub>").replace(/_(.)/g,"<sub>$1</sub>")}</code>`).replace(/\[(.*?)\]\((.*?)\)/g,(i,s,r)=>{const u=s.replace(/<[^>]*>?/gm,"");return`<a href="${r}" data-reader-link="true" data-url="${r}" data-title="${u}">${s}</a>`}).replace(/\*\*(.*?)\*\*/g,"<strong>$1</strong>").replace(/\*(.*?)\*/g,"<em>$1</em>");const a=t.split(`
...
`);return console.log("Parsed Markdown Result:",a),a}function ls(){const e=g,t=E(e.id);return`
  ...
  `}function cs(e){return!e.hints||e.hints.length===0?"":`
    <div class="hints-section">
      ...
      <div class="hint-accordion">
        ${e.hints.map(t=>`
          <div class="hint-item ${P.has(t.order)?"open":""}" data-hint="${t.order}">
  ...
  `}function ds(e){return`
    <div class="solution-section">
      <button class="solution-toggle" onclick="window.toggleSolution()">
        ...
        <span>${v?"Hide Solution":"Reveal Solution"}</span>
      </button>
      <div class="solution-content ${v?"open":""}">
  ...
  `}function Qi(){S&&clearInterval(S);const e=document.getElementById("countdown");e&&(S=setInterval(()=>{const t=Vi();e.textContent=t,t==="00:00:00"&&(clearInterval(S),S=null,D())},1e3))}function ps(){E(g.id)&&Qi()}function fs(){const e=rs(g.id);return`
  ...
  `}function gs(){return!p||f.length===0?"":`
    <div style="display: grid; grid-template-rows: ${d.map((t,o)=>o<d.length-1?`${t}fr 8px`:`${t}fr`).join(" ")}; height: 100%; width: 100%;">
  ...
  `}function Zi(){return g=ss(),`
    <header>
      <div class="logo">ATH<span>ENA</span></div>
      <div class="tagline">Daily Problem</div>
    </header>
    ${ys()}
  `}window.toggleHint=e=>{const t=document.querySelector(`.hint-item[data-hint="${e}"]`);t&&(P.has(e)?(P.delete(e),t.classList.remove("open")):(P.add(e),t.classList.add("open")))};window.toggleSolution=()=>{v=!v;const e=document.querySelector(".solution-toggle span:last-child"),t=document.querySelector(".solution-content");e&&(e.textContent=v?"Hide Solution":"Reveal Solution"),t&&(v?t.classList.add("open"):t.classList.remove("open"))};window.markSolved=e=>{as(e),D()};window.saveScratchpad=(e,t)=>{j(e,t)};window.handleNotesPaste=(e,t)=>{if(e.clipboardData&&e.clipboardData.files&&e.clipboardData.files.length>0){const o=e.clipboardData.files[0];if(o.type.startsWith("image/")){e.preventDefault();const a=new FileReader;a.onload=i=>{var u;const s=(u=i.target)==null?void 0:u.result;document.execCommand("insertImage",!1,s);const r=e.target;r&&j(t,r.innerHTML)},a.readAsDataURL(o)}}else if(e.clipboardData){const o=e.clipboardData.getData("text/plain");if(o){e.preventDefault(),document.execCommand("insertText",!1,o);const a=e.target;a&&j(t,a.innerHTML)}}};window.openReader=e=>{const t=g.concepts[e];window.openReaderByLink(null,t.resourceUrl,t.resourceTitle)};window.openReaderByLink=(e,t,o)=>{if(e&&e.preventDefault(),f.findIndex(s=>{var r;return((r=g.concepts[s])==null?void 0:r.resourceUrl)===t})===-1){const s=g.concepts.findIndex(u=>u.resourceUrl===t);if(s!==-1)f.push(s);else{const u=o.replace(/<[^>]*>?/gm,""),x={name:u,description:"",resourceTitle:u,resourceUrl:t,readTime:""};g.concepts.push(x),f.push(g.concepts.length-1)}const r=f.length;d=f.map(()=>100/r)}const i=p;p=f.length>0,!i&&p&&(n.length===2||n[2]===0)&&(n[0]>33?(n[0]-=33,n[2]=33):n=[33,34,33]),D()};document.addEventListener("click",e=>{const o=e.target.closest('a[data-reader-link="true"]');if(o){e.preventDefault();const a=o.dataset.url||"",i=o.dataset.title||"";window.openReaderByLink(null,a,i)}});window.closeReader=e=>{if(f=f.filter(t=>t!==e),p=f.length>0,!p)n=[n[0]+n[2],n[1],0];else{const t=f.length;d=f.map(()=>100/t)}D()};function D(){document.querySelector("#app").innerHTML=Zi(),bs(),E(g.id)&&Qi()}function bs(){const e=document.querySelectorAll(".resizer");let t=!1,o=null,a=0,i=[];const s=document.querySelectorAll(".v-resizer");let r=!1,u=null,x=0,b=[];s.forEach(c=>{c.addEventListener("dblclick",()=>{const l=parseInt(c.dataset.vindex||"0");d[l]<5?(d[l]=50,d[l+1]=50):(d[l+1]+=d[l]-5,d[l]=5);const h=c.parentElement;h&&(h.style.gridTemplateRows=d.map((y,w)=>w<d.length-1?`${y}fr 8px`:`${y}fr`).join(" "))}),c.addEventListener("mousedown",l=>{r=!0,u=c,x=l.clientY,b=[...d],document.body.style.cursor="row-resize",document.body.style.userSelect="none";const h=document.querySelector(".workspace");h&&(h.style.pointerEvents="none"),l.stopPropagation()})}),e.forEach(c=>{c.addEventListener("dblclick",()=>{const l=parseInt(c.dataset.index||"0"),h=5;l===0?n[0]<=h?(n[1]=n[1]+n[0]-(50-(p?n[2]/2:0)),n[0]=50-(p?n[2]/2:0)):(n[1]=n[1]+n[0]-h,n[0]=h):l===1&&p&&(n[2]<=h?(n[1]=n[1]+n[2]-33,n[2]=33):(n[1]=n[1]+n[2]-h,n[2]=h));const y=p?`${n[0]}fr 8px ${n[1]}fr 8px ${n[2]}fr`:`${n[0]}fr 8px ${n[1]}fr`,w=document.querySelector(".workspace");w&&(w.style.gridTemplateColumns=y)}),c.addEventListener("mousedown",l=>{t=!0,o=c,a=l.clientX,i=[...n],document.body.style.cursor="col-resize",document.body.style.userSelect="none";const h=document.querySelector(".workspace");h&&(h.style.pointerEvents="none")})}),document.addEventListener("mousemove",c=>{if(r&&u!==null){const m=parseInt(u.dataset.vindex||"0"),k=c.clientY-x,T=u.parentElement,to=(T==null?void 0:T.offsetHeight)||window.innerHeight,no=k/to*100,I=[...b],M=5;let B=b[m]+no;const io=b[m]+b[m+1]-M;B=Math.max(M,Math.min(io,B));const oo=B-b[m];I[m]=B,I[m+1]=b[m+1]-oo,d=I;const so=d.map((q,ao)=>ao<d.length-1?`${q}fr 8px`:`${q}fr`).join(" ");T&&(T.style.gridTemplateRows=so);return}if(!t||o===null)return;const l=parseInt(o.dataset.index||"0"),h=c.clientX-a,y=document.querySelector(".workspace"),w=(y==null?void 0:y.offsetWidth)||window.innerWidth,W=h/w*100,$=[...i],A=5;if(l===0){let m=i[0]+W;const k=100-A-(p?i[2]:0);m=Math.max(A,Math.min(k,m)),$[0]=m,$[1]=100-$[0]-(p?i[2]:0)}else if(l===1&&p){let m=i[1]+W;const k=100-i[0]-A;m=Math.max(A,Math.min(k,m)),$[1]=m,$[2]=100-i[0]-$[1]}n=$;const eo=p?`${n[0]}fr 8px ${n[1]}fr 8px ${n[2]}fr`:`${n[0]}fr 8px ${n[1]}fr`;y&&(y.style.gridTemplateColumns=eo)}),document.addEventListener("mouseup",()=>{r&&(r=!1,u=null),t&&(t=!1,o=null),document.body.style.cursor="",document.body.style.userSelect="";const c=document.querySelector(".workspace");c&&(c.style.pointerEvents="")})}document.querySelector("#app").innerHTML=Zi();ps();document.addEventListener("copy",e=>{const t=window.getSelection();t&&!t.isCollapsed&&e.clipboardData&&(e.clipboardData.setData("text/plain",t.toString()),e.preventDefault())});
